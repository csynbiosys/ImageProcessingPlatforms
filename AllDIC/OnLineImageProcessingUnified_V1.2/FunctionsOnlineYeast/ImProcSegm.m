...
function [] = ImProcSegm(obj, event, cutcor, pDIC, ident, INP, CitFreq, DicFreq, webPath, bacpat, cutcorBACK1, cutcorBACK2)
...
filePattern4 = fullfile(pDIC, 'exp*cy5_001.png');
...
for i=1:maxidS % Cut Sulforodamine images
    if ~isfile([pDIC,'\CutDye\',Files3(i).name])
        temp1 = imread([pDIC,'\',Files3(i).name]);
        temp2 = temp1(cutcor(1):cutcor(2),cutcor(3):cutcor(4));
        imwrite(temp2, [pDIC,'\CutDye\',Files3(i).name])
    end
end

for i=1:maxidCy % Cut Cy5 images
    if ~isfile([pDIC,'\CutDye\',Files4(i).name])
        temp1 = imread([pDIC,'\',Files4(i).name]);
        temp2 = temp1(cutcor(1):cutcor(2),cutcor(3):cutcor(4));
        imwrite(temp2, [pDIC,'\CutDye\',Files4(i).name])
    end
end
...
if ~isempty(cutcorBACK1)
    ...
    filePattern4b = fullfile(bacpat, 'exp*cy5_001.png');
    ...
    for i=1:maxidSb % Cut Sulforodamine images
        if ~isfile([bacpat,'\CutDyeBackOne\',Files3b(i).name])
            temp1 = imread([bacpat,'\',Files3b(i).name]);
            temp2 = temp1(cutcorBACK1(1):cutcorBACK1(2),cutcorBACK1(3):cutcorBACK1(4));
            imwrite(temp2, [bacpat,'\CutDyeBackOne\',Files3b(i).name])
        end
        if ~isfile([bacpat,'\CutDyeBackTwo\',Files3b(i).name]) && ~isempty(cutcorBACK2)
            temp1 = imread([bacpat,'\',Files3b(i).name]);
            temp2 = temp1(cutcorBACK2(1):cutcorBACK2(2),cutcorBACK2(3):cutcorBACK2(4));
            imwrite(temp2, [bacpat,'\CutDyeBackTwo\',Files3b(i).name])
        end
    end
    for i=1:maxidCyb % Cut Cy5 images
        if ~isfile([bacpat,'\CutDyeBackOne\',Files4b(i).name])
            temp1 = imread([bacpat,'\',Files4b(i).name]);
            temp2 = temp1(cutcorBACK1(1):cutcorBACK1(2),cutcorBACK1(3):cutcorBACK1(4));
            imwrite(temp2, [bacpat,'\CutDyeBackOne\',Files4b(i).name])
        end
        if ~isfile([bacpat,'\CutDyeBackTwo\',Files4b(i).name]) && ~isempty(cutcorBACK2)
            temp1 = imread([bacpat,'\',Files4b(i).name]);
            temp2 = temp1(cutcorBACK2(1):cutcorBACK2(2),cutcorBACK2(3):cutcorBACK2(4));
            imwrite(temp2, [bacpat,'\CutDyeBackTwo\',Files4b(i).name])
        end
    end
end
...
for i=1:maxidM % Cut DIC images
    if ~isfile([pDIC,'\Segmentation\BKG-',Files4(i).name])
        ...
        imwrite(uint16(z), [pDIC,'\Segmentation\BKG-',Files4(i).name])
    end
end

%% Compute background sulforhodamine levels
load([pDIC,'\Segmentation\TemporaryDye.mat'],'tSulf');
...
if ~isempty(cutcorBACK1)
    ...
    for i=1:maxidS % Loop if there is Sulforhodamine images
        if isnan(tSulf(1,i))
            num = num2str(r(i),'%.3u');
            sul = imread([bacpat,'\CutDyeBackOne\exp_000',num,'_Sulforhodamine_001.png']);
            ...
            if ~isempty(cutcorBACK2)
                ...
                sulb = imread([bacpat,'\CutDyeBackTwo\exp_000',num,'_Sulforhodamine_001.png']);
    ...
    for i=1:maxidCy % Loop if there is Cy5 images
        if isnan(tSulf(1,i))
            num = num2str(r(i),'%.3u');
            sul = imread([bacpat,'\CutDyeBackOne\exp_000',num,'_cy5_001.png']);
            sul21 = imread([bacpat,'\SegmentationOne\exp_000',num,'_DIC_001.tif']);    
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            % Patch for segmentation issues when there is no cells
            L = bwlabel(sul21,4);
            ...
            if ~isempty(cutcorBACK2)
                ...
                sulb = imread([bacpat,'\CutDyeBackTwo\exp_000',num,'_cy5_001.png']);
                sul21b = imread([bacpat,'\SegmentationTwo\exp_000',num,'_DIC_001.tif']);    
    ...
    for i=1:maxidS % Cut DIC images
        if isnan(tSulf(1,i))
            num = num2str(r(i),'%.3u');
            sul = imread([pDIC,'\CutDye\exp_000',num,'_Sulforhodamine_001.png']);
            sul21 = imread([pDIC,'\Segmentation\exp_000',num,'_DIC_001.tif']);
            tSulf(i)=median(sul(logical(sul21)));
        end
    end
    for i=1:maxidCy % Cut DIC images
        if isnan(tSulf(1,i))
            num = num2str(r(i),'%.3u');
            sul = imread([pDIC,'\CutDye\exp_000',num,'_cy5_001.png']);
            sul21 = imread([pDIC,'\Segmentation\exp_000',num,'_DIC_001.tif']);
            tSulf(i)=median(sul(logical(sul21)));
        end
    end
...
try
    saveas(gcf,[webPath,'\',ident,'-Citrine.png']);
catch
...
try
    ...
    if maxidS>0
        ...
        filePattern4 = fullfile(pDIC, 'exp*cy5_001.png');
        Files4 = dir(filePattern4);
        s = imread([Files4(end).folder, '\', Files4(end).name]);
    end
    ...
    if cutcor(4)-cutcor(3)>cutcor(2)-cutcor(1)
        ...
        title('Mask')
        hold off
        saveas(gcf, [webPath,'\',ident,'-LastTP1.png']);
        ...
        title('Dye')
        hold off
        saveas(gcf, [webPath,'\',ident,'-LastTP2.png']);
    ...
    RemoveUnparedImages(pDIC, bacpat);
    %open file
