...
function [x,y]=CorrectSegmentsFull(x, Device)
    ...
    switch Device
        case 'Yeast'
            for k=1:length(nuce) % Iterate over the number of cells in the frame
                ...
                if length(pc)<=175 % If the cell is small, put them on s and delete them from x. The cells will be added again at the end of the script. 
                    if length(pc) <=4 % Delete elements that are smaller than 20 pixels (segmentation error since cells are not that small)
                        x(pc)=0;
                    else
                        s(pc)=1;
                        x(pc)=0;
        ...
        case 'Bacteria'
            for k=1:length(nuce) % Iterate over the number of cells in the frame
                pc = find(L==nuce(k)); % Find matrix indexes for each cell
                if length(pc)<=210 % If the cell is small, put them on s and delete them from x. The cells will be added again at the end of the script. 
                    if length(pc) <=14 % Delete elements that are smaller than 20 pixels (segmentation error since cells are not that small)
                        x(pc)=0;
                    else
                        s(pc)=1;
                        x(pc)=0;
